...
-- This is a patched version which replaces the name tag by a localized
-- version if variable L10NLANG has been set to a target language
...
-- uncomment to localize names in desired target language -- currently only
-- latin script based languages are supported

-- Note that this will need a transcription daemon running on localhost
-- to work properly
...
-- Make string from get_names_from_tags array response
--
-- @param names get_names_from_tags output
-- @param delimiter delimiter to add between names
-- @return combined string
local function l10n_arr_to_str(names, delimiter)
    if names[1]=='' then
        return names[2]
    else
        if names[2]=='' then
            ...
            return names[1] .. delimiter .. names[2]
        end
    end
end


-- Generate localized name string from osm2pgsql object
--
-- @param object osm2pgsql object
-- @param islinear Object has either linear or point shaped label
...
--
local function prepare_columns(object, tag_map, ignore_type, islinear, iscountry)
...
local function process_node(object)
    local attrs = prepare_columns(object, columns_in_point_table, false, false, false)
    if attrs == nil then
        return
    end
...
local function process_way(object)
    ...
    local attrs = prepare_columns(object, columns_in_non_point_tables, false, true, false)
    if attrs == nil then
        return
    end
...
local function process_relation(object)
    local attrs = prepare_columns(object, columns_in_non_point_tables, false, false, false)
    ...
    if type == 'boundary' or (type == 'multipolygon' and object.tags.boundary) or type == 'route' then
        ...
        local attrs_linear
        -- for the sake of similarity to upstream code just re-prepare columns in some cases
        if (object.tags['name'] ~= nil) then
            if ((object.tags['admin_level'] == '2') and (object.tags['ISO3166-1:alpha2'] ~= nil)) then
                attrs = prepare_columns(object, columns_in_non_point_tables, false, false, true)
                attrs_linear = prepare_columns(object, columns_in_non_point_tables, false, true, true)
            else
                attrs_linear = prepare_columns(object, columns_in_non_point_tables, false, true, false)
            end
        else
            attrs_linear = attrs
        end
        
        add_linear('line', attrs_linear, geom)

        if in_roads then
            add_linear('roads', attrs_linear, geom)
        end
